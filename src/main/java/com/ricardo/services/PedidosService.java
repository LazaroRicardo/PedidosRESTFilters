...
public class PedidosService {
	...
	@DELETE
	@Produces("application/json")
	public Response borradoPedido(@PathParam("id") int pid) {
		if (pid > 0) {
			PedidosManager pm = PedidosManager.getInstance();
			return Response.status(200).entity(pm.borrarPedido(pid)).build();
		} else {
			return Response.status(404).entity(new StatusMessage(404, "el paquete no existe")).build();
		}
